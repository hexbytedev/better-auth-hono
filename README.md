...
Once running, `GET /api/health` returns `{ "status": "ok", ... }` for load-balancer / container health checks. Two interactive API references are served: `/api/auth/reference` (all Better-Auth endpoints, auto-generated by the `openAPI()` plugin) and — when the internal user API is enabled — `/api/users/reference` (the Basic-Auth lookup API).
...
- `src/routes/users.route.ts` - Basic-Auth protected internal user lookup (`GET /api/users/id/:id`, `POST /api/users/email`), defined with `@hono/zod-openapi` so it self-documents at `/api/users/reference`.
